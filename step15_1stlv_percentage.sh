...
while read -r subj; do
  for run in 1 2; do
    ...
    if [ -f "$subFile" ]; then
      ...
      awk -F'\t' 'NR==1{for(i=1;i<=NF;i++)h[$i]=i} NR>1{print $(h["trans_x"]) "," $(h["trans_y"]) "," $(h["trans_z"]) "," $(h["rot_x"]) "," $(h["rot_y"]) "," $(h["rot_z"])}' "$subFile" > "${subPath}/sub-${subj}_ses-01_task-${task}_run-00${run}_nuissance_regressors_for_GLM.csv"

      awk -F'\t' 'NR==1{for(i=1;i<=NF;i++)h[$i]=i} NR>1{print $(h["trans_x"]) " " $(h["trans_y"]) " " $(h["trans_z"]) " " $(h["rot_x"]) " " $(h["rot_y"]) " " $(h["rot_z"])}' "$subFile" > "${subPath}/sub-${subj}_ses-01_task-${task}_run-00${run}_nuissance_regressors_for_GLM_no_header.txt"

      # Extract outlier columns
      outlier_cols=$(head -1 "$subFile" | tr '\t' '\n' | grep -n "^motion_outlier" | cut -d":" -f1)

      if [ ! -z "$outlier_cols" ]; then
        cut -f$(echo "$outlier_cols" | paste -sd",") "$subFile" > "${subPath}/sub-${subj}_ses-01_task-${task}_run-00${run}_outliers.csv"
