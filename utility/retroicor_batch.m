function retroicor_batch(input_folder, output_folder, retroicor_code_dir)
% retroicor_batch(input_folder)
% retroicor_batch(input_folder, output_folder)
% retroicor_batch(input_folder, output_folder, retroicor_code_dir)
%
...
%
% Arguments:
%   input_folder       folder with co-located BOLD, JSON, and 1D files
%   output_folder      where to write corrected BOLDs (default: input_folder/../output)
%   retroicor_code_dir path to retroicor_main_modi.m etc. (default: same folder as this file)
%
% Outputs (ALL) are written into output_folder:
%   - corrected BOLD -> output_folder/<boldbase>_retro-corrected.nii.gz
%   - json/regressors/pctvar/phases -> output_folder/<boldbase>*

    if nargin < 1 || isempty(input_folder)
        error('retroicor_batch: input_folder is required.');
    end
    input_folder = char(input_folder);

    % Output folder: caller-supplied or sibling 'output' directory
    if nargin < 2 || isempty(output_folder)
        output_folder = fullfile(fileparts(input_folder), 'output');
    end
    output_folder = char(output_folder);

    % retroicor code dir: caller-supplied or same folder as this file
    if nargin < 3 || isempty(retroicor_code_dir)
        retroicor_code_dir = fileparts(mfilename('fullpath'));
    end
    retroicor_code_dir = char(retroicor_code_dir);
    ...
    output_corrected_folder = output_folder;
