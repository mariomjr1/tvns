...
#
# Usage: bash step01_times_acquisition.sh <json_dir> <mat_file> <output_dir> [python_exe]
#
#   json_dir    BIDS func directory holding the *_bold.json sidecars,
#               e.g. <sourcedata>/<subject>/ses-01/func
#   mat_file    Full path to the manually-selected LabChart .mat file
#   output_dir  Where pseudotime_mapping.json is written
#               (typically <sourcedata>/derivatives/physio/<subject>)
#   python_exe  Optional Python executable (default: python3)
#
# Files no longer need to be co-located: JSONs are read from the BIDS func
# directory, the .mat is given by absolute path, and output is written
# wherever you choose.
##############################################################################
...
def main():
    ...
    mat_file   = sys.argv[2]   # full path to the .mat
    ...
    # Parcels (pre-parsed per-sequence mats) are not present in the BIDS func
    # dir, so verification is normally skipped. Kept for backward compatibility
    # if parcels happen to sit alongside the JSONs.
    ...
    # ── STEP 5: Save output ────────────────────────────────
    # Store the ABSOLUTE .mat path so later steps (parse/plot) can locate it
    # even though it no longer lives next to the JSONs.
    output = {
