...
1. **Part 1 — Populate:** set the Step 07 first-level root (defaults to the **MNI route**,
   `derivatives/spm/first_level_mni`) + task root + contrast name (`wcon_0001.nii`) → **▶
   Run Part 1** (`step08a_populate_v2.sh`; verifies each subject's contrast by name and
   flags mismatches to `_contrast_check.csv` — review it).
